...
public class AddressValidator {
    public static void validateAddress(String address) {
        if(!address.matches("[A-Z][a-z]+\\ [1-9][0-9]{0,3}")){          //("[A-Z][a-z]+\\ [1-9]([0-9]{0,3})?([A-Za-z]{1})?(\\/[1-9][0-9]{0,2})?")
            throw new ValidationException("Address niepoprawny");
        }
    }
    ...
    public static void validateCity(String city) {
        ...
        if(!city.matches("[A-Z][a-z]+(-[A-Z][a-z]+)?(\\ [A-Z][a-z]+)?")){
            throw new ValidationException("Miasto niepoprawne");
        }
    }
